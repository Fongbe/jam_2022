...
Window::Window()
{
    ...
    if (!this->font.loadFromFile("assets/COMIC.TTF"))
        std::cerr << "Failed to load the font" << std::endl;
    
    if (!this->blueMusic.openFromFile("music/song.ogg"))
...
void Window::loop()
{
    ...
    this->cl.restart();
    this->blueMusic.play();
    this->blueMusic.setLoop(true);
    while (this->isWindowOpen()) {
        if (this->music == false)
            this->blueMusic.setVolume(0);
        if (this->music == true)
            this->blueMusic.setVolume(100);
        this->windowClear();
